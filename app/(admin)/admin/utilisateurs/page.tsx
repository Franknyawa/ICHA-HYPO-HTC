...
export default function UtilisateursPage() {
  ...
  const [sessionsList, setSessionsList] = useState<
    { id: string; userAgent: string | null; createdAt: string; lastSeenAt: string }[]
  >([]);
  ...
  async function openSessions(u: UserRow) {
    ...
    setSessionsList(d.data ?? []);
    setSessionsLoading(false);
  }

  async function revokeSession(sessionId: string) {
    await fetch(`/api/sessions/${sessionId}`, { method: "DELETE" });
    if (sessionsTarget) openSessions(sessionsTarget);
  }

  async function revokeAllSessions() {
    if (!sessionsTarget) return;
    await fetch(`/api/users/${sessionsTarget.id}/sessions`, { method: "DELETE" });
                ...
                {sessionsList.map((s) => (
                  ...
                  >
                    ...
                    <button
                      ...
                      className="shrink-0 rounded-lg bg-red-50 px-2 py-1 text-xs font-semibold text-alert"
                    >
                      Déconnecter
                    </button>
                  </div>
            ...
            <div className="flex gap-2">
              ...
              {sessionsList.length > 0 && (
                <button
                  ...
                  className="flex-1 rounded-lg bg-red-50 py-2 text-sm font-semibold text-alert"
                >
                  Déconnecter partout
                </button>
              )}
